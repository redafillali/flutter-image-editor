...
class SignatureController extends ValueNotifier<List<Point>> {
  ...
  Uint8List? _toPngBytesForWeb() {
    ...
    final Color pColor = img.getRowStride(penColor.red, penColor.green,penColor.blue as img.Format) as ui.Color;
    ...
    for (int i = 0; i < translatedPoints.length - 1; i++) {
      if (translatedPoints[i + 1].type == PointType.move) {
        img.drawLine(signatureImage,thickness: penStrokeWidth, x1: translatedPoints[i].offset.dx.toInt(), y1: translatedPoints[i + 1].offset.dx.toInt(), x2: translatedPoints[i].offset.dy.toInt(), y2: translatedPoints[i + 1].offset.dx.toInt(), color: img.ColorFloat16(16));
        ...
        img.fillCircle(signatureImage,    x: translatedPoints[i].offset.dx.toInt(), y: translatedPoints[i].offset.dy.toInt(),  radius: penStrokeWidth.toInt(), color: img.ColorFloat16(16));
